    ...
    public static void P2()
    {
        ...
        for (; i < lines.Length; i++)
        {
            ...
            if (failed)
            {
                ...
                {
                    var rule = ruleDict[value];
                    rule.SortValue = rule.MustBeAfter.Count(x => values.Contains(x));
                    ruleList.Add(rule);
                }

                var ordered = ruleList.OrderBy(x => x.SortValue).ToList();
                result += ordered[ordered.Count / 2].id;
